...
group = utah %>% group_by(Pop.Group) %>% summarize(Mean.Religiosity = mean(Religious))
...
group.pop = utah %>% group_by(Pop.Group) %>% summarize(Mean.Pop = mean(Pop_2010))
...
religiosity = tibble(Pop.Group=c("group1","group2","group3","group4","group5","group6"),
                     Mean.Religiosity=group$Mean.Religiosity, Mean.Pop=group.pop$Mean.Pop)
...
# Episcopal Church R^2 is .3557
# 4.  What is the direction of THAT correlation?
# Postive
# 5.  What can you say about these relationships?
# The correlation is in areas with more LDS there happens to be more Non_Religious, 
# and in areas with more Episcopal the Non_Religious is higher
