...
# Now, use this workflow to model miles per gallon in the mtcars dataset!
    ...
    # Things to consider:
      # What variables are explanatory? disp, hp, and wt have the strongest correlations
      # What interaction terms are useful? pretty much all of them have some influence, carb gear and cyl most
      # What's the simplest model that has good explanatory power? model4
...
mod4 <- lm(data=mtcars, formula = mpg ~ wt)
mod5 <- lm(data=mtcars, formula = mpg ~ wt + cyl)
mod6 <- lm(data=mtcars, formula = mpg ~ wt * cyl)
...
mod4mse ; mod5mse ; mod6mse


df_mod6 <- add_predictions(mtcars,mod6)
df_mod6

df2 <- gather_predictions(mtcars, mod4,mod5,mod6)
df2
skim(df2)
names(df2)


p10 <- ggplot(df_mod6,aes(x=wt,color=cyl)) +
  geom_point(aes(y=mpg),alpha=.5,size=2) +
  geom_point(aes(y=pred),color="Black") + theme_bw()
p10

p10 + geom_segment(aes(y=mpg,xend=wt,yend=pred),
                  linetype=2,color="Black",alpha=.5)

ggplot(df2,aes(x=wt,color=cyl)) + #i acutally like mod4, (i switched gear to cyl now maybe mod 6 actually), just tested it 4 looks better
...
mtcarstest <- add_predictions(test,mod4_cv)

ggplot(mtcarstest,aes(x=wt,color=cyl)) +
  geom_point(aes(y=mpg),alpha=.25) +
  geom_point(aes(y=pred),shape=5)
...
ggplot(df2, aes(x=wt,color=cyl)) +
  geom_point(aes(y=mpg),alpha=.2) +
  geom_smooth(method = "lm",aes(linetype=model,y=pred)) + theme_bw()

#Looks gucci to me, mod4 is the way to go
